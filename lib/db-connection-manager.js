...
class DatabaseConnectionManager {
  ...
  /**
   * Helper to create a prisma client with optional accelerate extension
   */
  createPrismaClient(url) {
    ...
    if (isAccelerate) {
      console.log(`[DB Manager] Applying Prisma Accelerate extension to ${this.maskConnectionString(url)}`);
  ...
  async connectToPrimary() {
    ...
    try {
      ...
      if (!this.primaryClient) {
        this.primaryClient = this.createPrismaClient(CONFIG.PRIMARY_URL);
      }
  ...
  async connectToSecondary() {
    ...
    try {
      ...
      if (!this.secondaryClient) {
        this.secondaryClient = this.createPrismaClient(CONFIG.SECONDARY_URL);
      }
  ...
  /**
   * Get the current Prisma client synchronously (used by proxy accessors)
   */
  getCurrentClient() {
    if (!this.currentClient) {
      throw new Error('Prisma client not initialized yet. Try awaiting getPrisma() before using the client.');
    }
    return this.currentClient;
  }
